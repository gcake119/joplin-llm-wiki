...
Define shipped macOS `launchd` LaunchAgent artifacts and operator workflows so **Ollama**, **Chroma**, and `joplin-brain sqlite-sync` (including optional Joplin CLI write-back) can be installed together with one operator-facing command, run continuously in the user session with predictable PATH, working directory, logging, bounded dependency readiness checks for `sqlite-sync`, and install/uninstall behavior, without introducing non-local services.
