...
dependencies、建立 `joplin-llm-wiki-mcp` 啟動 shim，並在指定時寫入
Codex/Cursor 的 MCP 設定；使用者不需要先手動 clone 這個專案。
...
安裝後，還需要在安裝目錄準備 `config.yaml`，並依自己的 Joplin 環境設定
Joplin SQLite、`raw`、`wiki`、Ollama、Joplin Data API / Web Clipper token
等必要欄位。最小啟動流程：
...
設定完成後，重啟 Codex/Cursor，或在 Cursor 執行 Reload Window，讓 MCP
client 重新載入 server 設定。
