...
`sqlite-sync` 的「定時檢查」是輪詢，不是 macOS 檔案事件 watcher。此 LaunchAgent plist 目前以 `RunAtLoad` 啟動 `run-sqlite-sync.sh`；是否持續每隔幾秒檢查，取決於 `config.yaml` 的 `joplin_sqlite_sync.schedule.every_seconds`。若該值為 `null`，這個 job 會跑完單輪後結束，除非你另行在 plist 加 `StartInterval` 或用其他排程重啟它。
...
| `joplin-llm-wiki-sqlite-sync` | **輪詢 Ollama 就緒**後執行 **`joplin-llm-wiki sqlite-sync`**；後續是否常駐輪詢由 `schedule.every_seconds` 決定 |
...
範本 plist **未**預設 `KeepAlive` 或 `StartInterval`，避免依賴異常時無限重啟灌爆日誌。建議優先用 `joplin_sqlite_sync.schedule.every_seconds` 讓 `sqlite-sync` 行程內輪詢；若你改用 `StartInterval` 反覆啟動單輪命令，請保持 `every_seconds: null`，避免外部排程與內部輪詢重疊。若你需要自動拉起，請自行在 plist 加入 `KeepAlive` 並理解可能之**重啟循環**；亦請注意 **Joplin Desktop** 與 **sqlite-sync** 同時存取 `database.sqlite` 時之鎖定行為（工具以唯讀開啟 SQLite，仍建議備份與錯峰）。
