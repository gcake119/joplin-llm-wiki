...
export async function runQuery(ctx) {
  ...
  const workflowRoot = path.dirname(path.resolve(ctx.configPath));

  const confirmId = ctx.opts.get("confirm-capture");
  if (confirmId) {
    return confirmCapture({ cfg, workflowRoot, id: confirmId, opts: ctx.opts });
  }

  const showId = ctx.opts.get("show-capture");
  if (showId) {
    return showCapture({ workflowRoot, id: showId });
  }

  const question = ctx.argv.join(" ").trim();
  if (!question) {
    console.error(JSON.stringify({ error: "QUERY_EMPTY", message: "question text required" }));
    return 1;
  }

  const provider = ctx.opts.get("provider") ?? "ollama";
  ...
  const sourceScope = readSourceScope(ctx.opts);
  ...
  if (wiki.files.length === 0 && raw.files.length === 0) {
    console.error(JSON.stringify({ error: "EMPTY_KNOWLEDGE", message: "query requires markdown files under wiki/ or raw/" }));
    return 1;
  }

  const maxContextChars = readPositiveIntOpt(ctx.opts, "max-context-chars", 24_000);
  const knowledge = buildKnowledgeContext({ wiki, raw, sourceScope }, maxContextChars);
  const prompt = `你是個人知識庫 query agent。你可以根據下方 KNOWLEDGE CONTEXT 回答，這代表使用者知識庫，而不是外部網路。
...
`;
  ...
  const answer =
    provider === "ollama"
      ...
      : await answerWithCodex(path.dirname(path.resolve(ctx.configPath)), prompt);

  const parsed = parseAnswerCapture(answer);
  const trimmed = parsed.answer.trim();
  console.log(trimmed);
  console.log(`SOURCES ${JSON.stringify(knowledge.sources)}`);

  const capture = buildCaptureDraft({
    requested: ctx.opts.get("capture"),
    legacyFileBack: ctx.opts.get("file-back"),
  ...
  if (capture) {
    const pending = writePendingCapture({
      workflowRoot,
      question,
      answer: trimmed,
      provider,
    ...
    console.log(`CAPTURE_DRAFT ${JSON.stringify({
      ...
      confirm: `joplin-llm-wiki query --config ${ctx.configPath} --confirm-capture ${pending.id}`,
    })}`);
    console.error(JSON.stringify({ capture_pending_id: pending.id, capture_pending_path: path.relative(process.cwd(), pending.path) }));
  }
...
/**
 * @param {{ cfg: import('../config/load-config.js').AppConfig, workflowRoot: string, id: string, opts: Map<string, string> }} args
 */
async function confirmCapture(args) {
  const pending = readPendingCapture(args.workflowRoot, args.id);
  if (!pending) {
    console.error(JSON.stringify({ error: "CAPTURE_NOT_FOUND", message: `pending capture not found: ${args.id}` }));
    return 1;
  }
  const capture = pending.capture;
  if (capture.classification !== "brainstorming" && capture.classification !== "artifacts") {
    console.error(JSON.stringify({ error: "CAPTURE_INVALID", message: "pending capture classification must be brainstorming or artifacts" }));
    return 1;
  }
  const dest = captureDestination(args.workflowRoot, capture.classification, capture.title, args.opts, args.cfg);
  if (!dest) return 1;
...
/** @param {{ workflowRoot: string, id: string }} args */
function showCapture(args) {
  const pending = readPendingCapture(args.workflowRoot, args.id);
  if (!pending) {
    console.error(JSON.stringify({ error: "CAPTURE_NOT_FOUND", message: `pending capture not found: ${args.id}` }));
    return 1;
...
function captureDestination(workflowRoot, classification, title, opts, cfg) {
  ...
  if (!project || !project.trim()) {
    console.error(JSON.stringify({ error: "ARTIFACT_PROJECT_REQUIRED", message: "artifacts capture requires --artifact-project or joplin_wiki_writeback.artifacts_project_notebook_title" }));
    return null;
  }
  const projectSlug = slugify(project).slice(0, 64) || "project";
  const rel = `artifacts/projects/${projectSlug}/${stamp}-${slug}.md`;
...
/**
 * @param {Map<string, string>} opts
 * @returns {"knowledge" | "wiki" | "raw"}
 */
function readSourceScope(opts) {
  const raw = opts.get("source-scope") ?? "knowledge";
  if (raw === "knowledge" || raw === "wiki" || raw === "raw") return raw;
