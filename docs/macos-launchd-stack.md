...
3. **`joplin-brain sqlite-sync`**（含每週期匯出、索引、`wiki-compile`／寫回等，由你的 `config.yaml` 決定）
...
| 服務 | 路徑 |
|------|------|
| Ollama | `~/Library/Logs/joplin-brain/ollama.log`、`ollama.err.log` |
| Chroma | `~/Library/Logs/joplin-brain/chroma.log`、`chroma.err.log` |
| sqlite-sync | `~/Library/Logs/joplin-brain/sqlite-sync.log`、`sqlite-sync.err.log` |
...
```bash
mkdir -p ~/Library/Logs/joplin-brain
chmod +x scripts/launchd/*.sh
REPO_ROOT="$(pwd)" JOPLIN_BRAIN_CONFIG="/絕對路徑/你的.config.yaml" \
...
若你已用手動終端機或其他方式常駐 Ollama／Chroma，可只複製／載入 `com.joplin-brain.sqlite-sync.plist`（仍需正確 `REPO_ROOT`、`JOPLIN_BRAIN_CONFIG` 與日誌路徑）；若 Ollama／Chroma 未起，`run-sqlite-sync.sh` 會依 `MLS_WAIT_TIMEOUT_SEC` **逾時失敗**。
...
```bash
pnpm exec joplin-brain sqlite-sync --config /絕對路徑/你的.config.yaml
```
