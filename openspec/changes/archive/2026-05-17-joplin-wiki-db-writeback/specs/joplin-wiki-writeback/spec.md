...
## ADDED Sections (supporting)

### Components & Interfaces

| Name | Input | Output | Errors | Idempotent |
| --- | --- | --- | --- | --- |
| JoplinWikiWriteback | AppConfig, touched wiki paths, dryRun flag | JSON summary counters | CONFIG_INVALID, JOPLIN_CLI_FAILED, JOPLIN_CLI_WRITE_FAILED | Same inputs yield same final Joplin tree for the same wiki set |

### Data Flow

```mermaid
sequenceDiagram
  participant WC as wiki-compile
  participant FS as wiki_root
  participant WB as JoplinWikiWriteback
  participant JC as Joplin CLI

  WC->>FS: write compiled md
  WC->>WB: runWriteback if enabled and not dry-run
  WB->>JC: ensure parent notebook (note-wiki)
  WB->>JC: ensure topic notebooks + note upsert(body)
```

### Acceptance Tests

Acceptance tests SHALL mock subprocess invocation and assert that dry-run performs zero mutating spawns.

Acceptance tests SHALL assert notebook resolution, topic normalization, and upsert spawn patterns for at least one parent title and two distinct domains.

### Risks & Assumptions

The README SHALL instruct operators to back up the Joplin profile before running wiki-compile with writeback enabled (default on).

The README SHALL satisfy REQ-JWKB-README-PREREQUISITES Operator install documentation for Desktop versus CLI responsibilities.

Joplin CLI command syntax drift across Joplin versions SHALL be handled in implementation while keeping requirement identifiers stable.
