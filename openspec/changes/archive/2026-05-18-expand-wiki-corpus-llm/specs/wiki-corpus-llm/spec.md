...
Define **notebook-wide thematic LLM wiki** behavior for `wiki-compile`: **corpus augmentation defaults on when `wiki_ingest.corpus_mode_enabled` is omitted**, using a wider `notes_root` digest plus per-target excerpt bundles that optionally consult **`collection_sources`**, bounded by **`corpus_digest_max_files`**, **`max_pages_per_run`**, and **`corpus_chroma_top_k`**, without introducing remote SaaS by default configuration.
