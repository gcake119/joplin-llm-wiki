...
此目錄之 shell／plist 範本僅用於**本機** `launchd` 部署：透過 LaunchAgent 常駐 **Ollama**、**Chroma** 與 **`joplin-llm-wiki sqlite-sync`**，不對外公開新的服務介面；實際監聽仍為本機 loopback（與專案 README 一致）。
...
| `run-sqlite-sync.sh` | 等待 Ollama／Chroma 就緒後執行 `pnpm exec joplin-llm-wiki sqlite-sync` |
